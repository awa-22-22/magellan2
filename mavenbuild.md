...
This compiles all sources, runs the unit tests, and creates

```
target/magellan2-2.1.2.jar
```
...
The jar is executable (`Main-Class: magellan.client.Client`):

```bash
java -jar target/magellan2-2.1.2.jar
```

Run it from the project root directory. Like the Ant release layout, Magellan
reads its configuration and data files (`etc/`, `help/`) from the working
directory at runtime — only the classpath is bundled into the jar.
...
| `build_help`, `index_help` | runs the JavaHelp `Indexer` for `help/de` and `help/en`, packs `magellan-help.jar` | `maven-antrun-plugin` (`java` task) or `exec-maven-plugin` |
| `copy_release_data` | assembles the `release/` directory (lib, doc, etc, plugins, readme files, start scripts) | `maven-assembly-plugin` |
| `zip_release` | packs `release/` into `magellan_vX.Y.Z.zip` | `maven-assembly-plugin` |
